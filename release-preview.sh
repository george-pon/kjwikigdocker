...
pushd helm-chart
    ...
    if [ -z "$PRESENT" ] ; then
        ...
        helm install kjwikigdocker --name kjwikigdocker --set image.repository=${IMAGE_PREFIX}kjwikigdocker --set image.tag=stable --set image.pullPolicy=IfNotPresent
    else
        # use local image name
        helm upgrade kjwikigdocker kjwikigdocker --set image.repository=${IMAGE_PREFIX}kjwikigdocker --set image.tag=stable --set image.pullPolicy=IfNotPresent
    fi
    # wait for deploy
